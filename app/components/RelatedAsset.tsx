...
const relatedAsset: React.FC<realtedAssetProp> = ({ assetCount }) => {
  const heading = "Options1";
  const fileType = "MP4";
  const duration = "2 min";
  const date = "30/01/2024";
  ...
  return (
    ...
    <div className="p-8 w-4/5 absolute xl:left-32 sm:left-32 left-12 mt-44 xl:mt-28">
      <h1 className="text-xl font-bold">Related Asset({assetCount})</h1>
      <div className="lg:w-96 w-64 mt-2 border-drop-shadow-xl border rounded-lg">
        ...
        <div className="flex p-4">
          <div className="w-6 cursor-pointer h-6 pl-2 pt-0.5 rounded-full border-2 border-solid border-red-500 text-red-500 flex" onClick={()=>setModalState(true)}>
        ...
        <div className="flex p-4">
          <p>
            {fileType} - {duration} - Published {date}
          </p>
        </div>
      </div>
    </div>
